...
const contentCards = [
    ...
    {   
        ...
        description: "LESSON A 11 - 20L | UPDATING IN PROGRESS...", 
...
const AccessControlManager = (() => {
    // Check if user has access to a specific file/link
    const hasAccessToContent = (contentId) => {
        const currentUser = UserManager.getCurrentUser();
        
        // If no user or guest, check guest permissions
        if (!currentUser || currentUser.isGuest) {
            return currentUser && currentUser.access && currentUser.access.includes(contentId);
        }
        
        // Check permanent access
        if (currentUser.access && currentUser.access.includes(contentId)) {
            return true;
        }
        
        // Check timed access
        const fileConfig = currentUser.timedAccessConfig ? currentUser.timedAccessConfig[contentId] : null;
    ...
    const validateAccess = (contentId, redirectUrl = "https://www.rosankc.com.np") => {
        if (!hasAccessToContent(contentId)) {
            // Show notification
            NotificationManager.showNotification(
                "Access Denied", 
                "You don't have permission to access this content. Redirecting to homepage...", 
                "danger",
                3000
            );
    ...
    const getContentIdFromUrl = (url) => {
        // Check for exam parameter
        if (url.searchParams.get('exam')) {
            return url.searchParams.get('exam');
        }
        
        // Check for known content paths
        const path = url.pathname;
        if (path.includes('RX-STUDIO-KAPALI_TAMSUK_GENERATOR.html')) return 'file1';
        if (path.includes('Kapali.html')) return 'file4';
        if (path.includes('RX-S-QR.html')) return 'file3';
    ...
    const init = () => {
        // Check if current page requires access validation
        const currentUrl = new URL(window.location.href);
        const contentId = getContentIdFromUrl(currentUrl);
        
        if (contentId) {
            validateAccess(contentId);
        }
    };
...
const UserManager = (() => {
    ...
    return {
        ...
        hasAccessToFile,
        getRemainingTime,
        hasAccessToContent: AccessControlManager.hasAccessToContent
    };
})();

// media-controller.js - COMPLETE WORKING VERSION (FIXED)
const MediaController = {
    ...
    setupImageModal: function() {
        ...
        // FIXED: Use event delegation with capture phase and stop propagation
        document.addEventListener('click', (e) => {
            // Check if clicking on image or its parent/child elements
            const target = e.target.closest('.content-image');
            if (target) {
                e.preventDefault();
        ...
        }, true); // Use capture phase to intercept before option selection
        
        // Also handle images inside content-audio containers
        document.addEventListener('click', (e) => {
            const target = e.target.closest('.content-audio img.content-image');
            if (target) {
                e.preventDefault();
                e.stopPropagation();
                this.openImageModal(target.src);

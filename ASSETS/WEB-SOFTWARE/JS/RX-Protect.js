...
function enforceLogin() {
    // --- FIXED: Changed from sessionStorage to localStorage for permanent login ---
    const currentUserJSON = localStorage.getItem('currentUser');
    const body = document.body;
    
    if (!currentUserJSON) {
        // User is not logged in, show the redirect message.
        showLoginRequiredMessage();
    } else {
        // User IS logged in.
        // 1. Reveal the protected page content.
        body.classList.remove('rx-protect-hidden');
        
        // --- NEW: Show a welcome message to the user ---
        try {
            const user = JSON.parse(currentUserJSON);
            const userName = user.name || user.username; // Use name, or fallback to username
...
// --- NEW: Function to display the welcome toast message ---
function showWelcomeToast(userName) {
    // Create the toast element
    const toast = document.createElement('div');
    toast.className = 'rx-welcome-toast';
    // Use innerHTML to easily include an icon (requires Font Awesome)
    toast.innerHTML = `<i class="fas fa-check-circle"></i> Welcome, <strong>${userName}</strong>!`;

    // Add it to the page body
    document.body.appendChild(toast);

    // After a delay, add the fade-out class
    setTimeout(() => {
        toast.classList.add('fade-out');
        
        // Remove the element from the DOM completely after the animation finishes
        setTimeout(() => {
            toast.remove();
        }, 500); // This duration must match the fade-out animation time
    }, 3500); // The toast will be visible for 3.5 seconds
}
...
function showLoginRequiredMessage() {
    ...
    textElement.textContent = ''; // For typewriter effect
...
function addStyles() {
    ...
    style.textContent = `
        ...
        /* --- NEW STYLES for the Welcome Toast --- */
        .rx-welcome-toast {
            position: fixed;
            bottom: 20px;
            left: 50%;
        ...
        .rx-welcome-toast.fade-out {
            animation: slideOutDown 0.5s ease-in forwards;
        }
        .rx-welcome-toast i {
            color: #4CAF50; /* Green checkmark icon */
        }
        ...
        /* --- NEW KEYFRAMES for the Welcome Toast animation --- */
        @keyframes slideInUp {
...
// Wait for the DOM to be ready before running the check

...
function setupEventListeners() {
    ...
    // UPDATED: Event listeners for the new modal
...
function displayResults(results) {
    contentDisplay.innerHTML = `<div class="result-container">
        ...
        <a href="#" onclick="generateResultPDF()" class="download-btn"><i class="fas fa-download"></i> Download Result</a>
    </div>`;
...
function generateResultPDF() {
    ...
    printWindow.document.write(`<html><head><title>Exam Result - ${studentInfo.serial}</title>
        <style>body{font-family:Arial,sans-serif;margin:20px} .header{text-align:center;margin-bottom:20px} .meta{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px} .summary{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;text-align:center}</style></head>
        <body><div class="header"><h1>Rosan Xettri Studio</h1><h2>Korean Language Exam Result</h2></div>
        <div class="meta"><div><strong>Student:</strong> ${studentInfo.name}</div><div><strong>Exam ID:</strong> ${studentInfo.serial}</div><div><strong>Date:</strong> ${results.dateTime.fullDateTime}</div><div><strong>Duration:</strong> ${Math.floor(results.duration / 60)}m ${results.duration % 60}s</div></div>
        <div class="summary"><div><h3>${results.score}%</h3><p>Score</p></div><div><h3>${results.correct} / ${results.total}</h3><p>Correct</p></div><div><h3>${results.attempted}</h3><p>Attempted</p></div></div>
...
function handleScreenResize() {
    ...
    document.querySelectorAll(".question-image").forEach(img => {
        img.style.maxHeight = isMobile ? '200px' : '300px';
    });
}

...
function initializeExamPage() {
    ...
    // FIXED: Use the correct property names from user object
    studentInfo.id = user.id;
    studentInfo.name = user.fullName;    // Changed from user.name to user.fullName
    studentInfo.username = user.userName; // Changed from user.username to user.userName
    studentInfo.image = user.image;
    studentInfo.serial = `${String(user.id).padStart(2, '0')}`;
...
function buildResultsSummaryHTML(results) {
    ...
    return `<div class="result-container">
        ...
        <div class="result-actions">
            ...
            <button class="download-btn" onclick="window.location.href='Resource.html'">

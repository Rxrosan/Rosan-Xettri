// RX Access Manager - Independent Access Control System
// Works without modifying existing files
// Enhanced with day-based expiration and Formspree integration

const RXAccessManager = (function() {
    // Access configuration with pricing and day-based access
    let accessConfig = {
        1: { // ROSAN KC (admin)
        ...
        //2: { // John Smith
         //   accessType: 'mixed',
          //  resources: {
          //      1: { purchased: '2024-01-15', price: 499, expires: '2024-06-15' },
          //      8: { purchased: '2024-01-15', price: 299, expires: '2024-12-31' },
         //       9: { purchased: '2024-01-15', price: 199, expires: '2024-03-15' }
           // }
       // },
       // 3: { // Emma Johnson
        //    accessType: 'mixed',
        //    resources: {
        //        1: { purchased: '2024-02-01', price: 499, expires: '2024-05-01' }
         //   }
       // }
    };

    // Resource pricing information with day-based durations
    const resourcePrices = {
        1: { basePrice: 499, currency: "Rs ", days: 30 },
        2: { basePrice: 999, currency: "Rs ", days: 30 },
        3: { basePrice: 999, currency: "Rs ", days: 30 },
        4: { basePrice: 999, currency: "Rs ", days: 30 },
        5: { basePrice: 999, currency: "Rs ", days: 30 },
        6: { basePrice: 999, currency: "Rs ", days: 30 },
        7: { basePrice: 949, currency: "Rs ", days: 30 },
        8: { basePrice: 199, currency: "Rs ", days: 30 },
        9: { basePrice: 199, currency: "Rs ", days: 30 }
    };
    ...
    // Get current user ID - Improved version
    ...
    function loadFromLocalStorage() {
        try {
            const savedConfig = localStorage.getItem('rxAccessConfigV3'); // Updated version
            if (savedConfig) {
    ...
    // Check if user has access to resource (now uses days)
    ...
    // Get user's resource access status (updated for days-based)
    ...
    // Show notification message with custom color
    ...
    // Grant access function (updated for days-based)
    ...
    return {
        ...
        showNotification: showNotificationMessage // Add to public API
    };
})();
...
function initializeAccessManager() {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            setTimeout(() => RXAccessManager.init(), 1500);
        });
    } else {
        setTimeout(() => RXAccessManager.init(), 1500);
    }
...
window.addEventListener('storage', function(e) {
    if (e.key === 'rxCurrentUserId') {
        console.log('RX Access Manager: Login state changed in localStorage, refreshing...');
        setTimeout(() => {
            if (window.RXAccessManager && window.RXAccessManager.refreshLoginState) {
                window.RXAccessManager.refreshLoginState();
            }
        }, 500);
    }
});
...
window.RXAccessManager = RXAccessManager;
console.log('RX Access Manager: Script loaded and ready');